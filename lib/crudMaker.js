    ...
    router.get(
      ...
      async (req, res, next) => {
        try {
          ...
          const extraConditions = {}
          const ref = model.find()
          const sorts = config.methodsDef?.getall?.sortFields || []
          ...
          if (sort && sorts.length)
            finalSorts = sort
              ...
              .map((item) => {
                ...
                if (value > 1 || value < -1) return { [key]: 0 }
                return { [key]: value }
              })
              .reduce((acc, item) => {
          ...
          console.log({ finalSorts })
          // sort
          if (finalSorts && Object.keys(finalSorts)?.length) {
            ref.sort(finalSorts)
          } else if (config.methodsDef?.getall?.sort) {
            const { sort } = config.methodsDef?.getall
            ref.sort(sort)
          }

          ref.collation({ locale: 'en', numericOrdering: true })
